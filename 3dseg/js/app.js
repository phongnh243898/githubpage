...
const polyMgr = new Polygon3dManager({
  ...
  config: { vertexSize: 10, thickness: 2 }
});

// Video bridge: reuse load-video3d input for custom UI input
const videoFileInput = document.getElementById('videoFile');
const videoProxyInput = document.getElementById('load-video3d');
if (videoFileInput && videoProxyInput) {
  videoFileInput.addEventListener('change', () => {
    const f = videoFileInput.files?.[0];
    if (!f) return;
    videoProxyInput.files = videoFileInput.files;
    videoProxyInput.dispatchEvent(new Event('change', { bubbles: true }));
  });
}
